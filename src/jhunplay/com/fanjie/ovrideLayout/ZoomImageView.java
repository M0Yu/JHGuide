...
//				currentBitmapHeight = bitmapHeight;
				...
				float ratio1 = width / (bitmapWidth * 1.0f);
            	float ratio2 = height / (bitmapHeight * 1.0f);
            	if (ratio1 >= ratio2){
            		matrix.postScale(ratio2, ratio2);
            		float translateX = (width - (bitmapWidth * ratio2)) / 2f;
            		matrix.postTranslate(translateX, 0);
            		totalTranslateX = translateX;
            		totalRatio = initRatio = ratio2;
//            		currentBitmapWidth = bitmapWidth * ratio2;
//            		currentBitmapHeight = bitmapHeight * ratio2;
            	} else {
            		matrix.postScale(ratio1, ratio1);
            		float translateY = (height - (bitmapHeight * ratio1)) / 2f;
            		matrix.postTranslate(0, translateY);
            		totalTranslateY = translateY;
            		totalRatio = initRatio = ratio1;
//            		currentBitmapWidth = bitmapWidth * ratio1;
//            		currentBitmapHeight = bitmapHeight * ratio1;
            	}          	
            }  
            canvas.drawBitmap(sourceBitmap, matrix, null);  
        }  
