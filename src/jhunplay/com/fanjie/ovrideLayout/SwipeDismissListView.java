...
public class SwipeDismissListView extends ListView {
	...
	private void handleActionUp(MotionEvent ev) {
		...
		if (Math.abs(deltaX) > mViewWidth / 10) {
			dismiss = true;
...
//					});
	...
	private void performDismiss(final View dismissView, final int dismissPosition) {
		...
		animator.addListener(new AnimatorListenerAdapter() {
			@Override
			public void onAnimationEnd(Animator animation) {
				if (onDismissCallback != null&&dismissPosition>=0) {
					onDismissCallback.onDismiss(dismissPosition);
				}else if(onDismissCallback != null&&dismissPosition==-1){
					onDismissCallback.onDismiss(0);
				}
